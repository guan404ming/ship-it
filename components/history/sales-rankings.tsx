...
export function SalesRankings({ productSalesData }: SalesRankingProps) {
  ...
  const topSalesList = React.useMemo(() => {
    return [...aggregatedData]
      .sort((a, b) =>
        rankingMetric === "sales" ? b.sales - a.sales : b.quantity - a.quantity
      )
      .slice(0, 5);
  ...
  const bottomSalesList = React.useMemo(() => {
    return [...aggregatedData]
      .sort((a, b) =>
        rankingMetric === "sales" ? a.sales - b.sales : a.quantity - b.quantity
      )
      .slice(0, 5);
  }, [aggregatedData, rankingMetric]);
  ...
  const topGrowthList = React.useMemo(() => {
    return [...aggregatedData]
      .filter(
        (product) => product.growth !== undefined && product.growth !== null
      )
    ...
    <>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center mb-6">
        ...
        <div className="flex items-center">
          <ToggleGroup
            ...
            value={rankingMetric}
            onValueChange={(value) =>
              value && setRankingMetric(value as "sales" | "quantity")
            }
            variant="outline"
          >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="border shadow-sm">
          <CardHeader>
            <CardTitle>
              {rankingMetric === "sales" ? "銷售額" : "銷售量"}最高前五名
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
        ...
        <Card className="border shadow-sm">
          <CardHeader>
            <CardTitle>
              {rankingMetric === "sales" ? "銷售額" : "銷售量"}最低前五名
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
        ...
        <Card className="border shadow-sm">
          ...
          <CardContent>
            <p>
              ...
              <br />
              數據選擇：可切換銷售額或銷售量來檢視不同指標的排名
              <br />
              成長率計算：成長率表示該商品在所選時間區間與前一個相同長度時間區間相比的變化率
              <br />
              規格數：表示同一商品下不同規格的數量
            </p>
          </CardContent>
